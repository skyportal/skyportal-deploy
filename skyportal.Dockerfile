...
ADD skyportal /skyportal
COPY extensions/skyportal/ /skyportal
RUN ls -l /skyportal
WORKDIR /skyportal
...
COPY skyportal.config.yaml /skyportal/config.yaml
...
CMD ["bash", "-c", "source /skyportal_env/bin/activate && (make log &) && make run_production"]
